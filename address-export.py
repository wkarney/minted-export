...
address_book['address'] = [' '.join(street.split())
                           for street in street_details]
...
address_book = address_book.reindex(
    columns=[col.lower() for col in column_titles])

address_book.to_excel('./data/minted-addresses.xlsx', header=column_titles)
address_book.to_csv('./data/minted-addresses.csv',
                    index=False, header=column_titles)
